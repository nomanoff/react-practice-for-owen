...
export const todoSlice = createSlice({
  ...
  reducers: {
    ...
    //   state.todos  = [action.payload.title,...state.todos]; // Use this instead
    // },
  ...
  extraReducers: (builder) => {
    ...
    .addCase(createPostsAsync.fulfilled, (state, action) => {
      console.log("새로운 post 생성 시도 했어!");
      state.todos = [action.payload.title,...state.todos];
    });
  },
});
